...
export default function RecipeCard({ recipe, variant = "default" }) {
  // Handle different recipe data structures & enforce DISH_REGISTRY as Single Source of Truth
  const getRecipeData = () => {
    let title = "";
    let rawImage = "";
    let href = "";
    ...
    return {
      title,
      image: finalImage || DEFAULT_CARD_IMAGE,
      region,
      href,
      showImage: true,
    ...
    return (
      <Card className="rounded-none border-stone-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
        ...
        <CardHeader>
          ...
          <CardTitle className="text-xl font-serif font-bold text-stone-900 text-center">
            {data.title}
          </CardTitle>
          {data.region && (
            <p className="text-xs font-semibold text-rose-700 text-center mt-1">
              {data.region}
            </p>
          )}
