...
import { getCategoryEmoji, getCountryFlag, ALLOWED_CUISINES } from "@/lib/data";
...
export default async function DashboardPage() {
  ...
  const rawAreas = areasData?.areas || [];
  const uniqueAreasMap = new Map();
  rawAreas.forEach((item) => {
    if (item?.strArea && !uniqueAreasMap.has(item.strArea)) {
      uniqueAreasMap.set(item.strArea, item);
    }
  });
  const areas = Array.from(uniqueAreasMap.values());
